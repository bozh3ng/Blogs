...
$\operatorname{Nat}(\mathbf{C}(c,-), F)$ is set-size, but its member $\alpha$ is a class-size family $\{\alpha_x\}$. The gigantic family $\{\alpha_x\}$ is determined by a single set element  $\alpha_c\left(1_c\right)$. This is not just a technicality -- it is a remarkable compression: a family indexed by a proper class (all objects of $\mathbf{C}$) is completely encoded by one element of one set ($Fc$).

# A concrete example
Consider the category $\mathbf{FinSet}$ of finite sets. Let $c = \{*\}$ (a one-element set). The representable functor $\mathbf{FinSet}(\{*\}, -)$ sends a set $X$ to the set of functions $\{*\} \to X$, which is just $X$ itself. So $\mathbf{FinSet}(\{*\}, -) \cong \mathrm{Id}$ is the identity functor.

Now take any functor $F: \mathbf{FinSet} \to \mathbf{Set}$. The Yoneda Lemma says:
$$\operatorname{Nat}(\mathbf{FinSet}(\{*\}, -), F) \cong F(\{*\})$$
A natural transformation from the identity functor to $F$ is completely determined by a single element of $F(\{*\})$. "Probing with a point determines the natural transformation."
