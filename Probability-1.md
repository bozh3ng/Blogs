...
$$
Note: Independence is expensive. For $n$ (different) nontrivial and independent binary events, the size of sample space $|\Omega|\geq2^n$. More generally, if each RV takes $k$ values, we need $|\Omega| \geq k^n$.
...
- $\mathbb{P}(\{x\})$ is the probability of the singleton set $\{x\}$
...
One caveat: All the statements need $g(x)$ to be bounded or well-behaved so $\mathbb{E}[|g(X)|]<\infty$.
